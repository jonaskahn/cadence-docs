...
The Admin → Pool dashboard polls `GET /api/admin/pool/stats` and shows total instances, hot tier
...
The API uses standard Python logging from `cadence.main` (text lines by default). Ship **stdout** to your aggregation stack (ELK, Loki, CloudWatch, etc.) using your platform's log driver. For OpenTelemetry log export, configure `otel.logs_enabled` and related keys — see [Observability](/features/observability/).
