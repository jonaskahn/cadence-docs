...
```python title="cadence/api/api_key/router.py"
def _validate_scopes(scopes: List[str], session) -> None:
    for s in scopes:
        if s not in PERMISSIONS:
            raise HTTPException(422, detail=f"Unknown scope/permission: {s}")
        if session.is_sys_admin:
            continue
        if session.has_permission(s, None):
            continue
        org_ok = any(session.has_permission(s, oid) for oid in session.membership_org_ids)
        if not org_ok:
            raise HTTPException(403, detail=f"Cannot grant scope you do not hold: {s}")
```
