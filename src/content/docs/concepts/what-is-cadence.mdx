...
- **Orchestrator instances** — Configurable agent runtimes (LangGraph, Google ADK, OpenAI Agents SDK) with a **pool** that loads **hot-tier** instances at startup and reacts to lifecycle events.
...
The service is a **FastAPI** application (version **2.0.3**). Persistent state lives primarily in **PostgreSQL**; **Redis** backs sessions, rate limiting, and stats; **RabbitMQ** drives orchestrator lifecycle events when available; **S3 or MinIO** stores plugin ZIPs when enabled (`CADENCE_PLUGIN_S3_ENABLED=true`).
