...
```mermaid
flowchart TD
    UP[POST /upload — multipart ZIP] --> VAL[read_validated_plugin_file<br>magic bytes PK\x03\x04 + .zip extension + ≤5 MB]
...
2. `read_validated_plugin_file` validates the file: extension must be `.zip`, first 4 bytes must be `PK\x03\x04` (ZIP magic bytes), and size must not exceed 5 MB.
...
```python title="cadence/api/plugins/plugin.py"
...
org = await tenant_service.get_org(context.org_id)
org_domain = org.get("domain") if org else None
...
| Symptom                                             | Cause                                                       | Fix                                                                   |
| --------------------------------------------------- | ----------------------------------------------------------- | --------------------------------------------------------------------- |
| `400 Invalid plugin file`                           | Wrong file extension, bad magic bytes, or file > 5 MB       | Use `.zip` file under 5 MB; re-zip if file is corrupted               |
...
<CardGrid>
  <LinkCard
    ...
    href="/guides/plugin-upload-pipeline/"
    description="HTTP to storage: zip guards, AST scan, subprocess inspector, dependencies, catalog, and events."
  />
  <LinkCard
    title="Orchestrator load, plugins, and settings"
    href="/guides/orchestrator-load-and-plugin-settings/"
    description="Pool cold load, factory.create, plugin resolution, settings injection, and bundle cache."
  />
  <LinkCard
    title="AI Agent SDK"
