...
**Primary modules:** `cadence/api/tenant/organizations.py` — member org routes and `/api/admin/orgs*`; `cadence/api/tenant/membership.py` — `/api/orgs/{org_id}/members` and `/users` membership routes.
