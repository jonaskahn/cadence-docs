...
Choosing a backend means choosing the engine that will execute your agent graph — how tool calls get routed, how agents hand off to each other, and which model providers the framework can talk to. Cadence currently runs two active backends: `langgraph` and `openai_agents`. A third framework type, `google_adk`, is defined in the platform's constants and provider registry but is not registered in the backend factory and cannot be used to create orchestrator instances.
...
| Framework       | Supported modes                                    | Provider support               |
| --------------- | -------------------------------------------------- | ------------------------------ |
| `langgraph`     | `supervisor`, `coordinator`, `handoff`, `grounded` | All 10 providers               |
| `openai_agents` | `supervisor`, `coordinator`, `handoff`             | `openai`, `litellm`, `bifrost` |

`langgraph` uses the LangChain ecosystem and supports stateful graphs with all registered providers. `openai_agents` uses the OpenAI Agents SDK swarm model and is limited to providers that implement the OpenAI chat completions interface.
