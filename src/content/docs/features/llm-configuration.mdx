...
**Instance resolution** — At runtime, `LLMModelFactory.create_model_by_id` fetches the org's `OrganizationLLMConfig` by ID, decrypts the API key, validates the provider, and constructs the framework-appropriate model object.
